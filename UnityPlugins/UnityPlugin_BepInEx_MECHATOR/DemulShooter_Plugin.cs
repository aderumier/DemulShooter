...
namespace BepInEx_DemulShooter_Plugin
{
    ...
    {
        public const String pluginGuid = "argonlefou.demulshooter.mechatornado";
        public const String pluginName = "MechaTornado_BepInEx_DemulShooter_Plugin";
        public const String pluginVersion = "17.0.0.0";
        public const String pluginConfigFile = "MechaTornado_BepInEx_DemulShooter_Plugin.ini";
        ...
        public static PluginControllerButton Left_Key = new PluginControllerButton((int) KeyCode.LeftArrow);
        public static PluginControllerButton Right_Key = new PluginControllerButton((int) KeyCode.RightArrow);
        ...
        public static bool EnableTriggerWaterDelay = false;

        //Custom data        
        public static int[] BufferCoin = new int[4];
        public static int[] BufferTicket = new int[4];
        ...
        public void Awake()
        {
            ...
            if (File.Exists(Plugin_IniFile.FInfo.FullName))
            {
                try
                {
                    Plugin_IniFile.IniReadIntValue("System", "PLAYERS", ref PlayerNum);
                    Plugin_IniFile.IniReadBoolValue("System", "USE_WATERGUN", ref EnableTriggerWaterDelay);
                    ...
                    Left_Key.SetKeyCode(Plugin_IniFile.IniReadValue("INPUT_KEYS", "LEFT"));
                    Right_Key.SetKeyCode(Plugin_IniFile.IniReadValue("INPUT_KEYS", "RIGHT"));
                }
                catch (Exception Ex)
                {
        ...
        public void Update()
        {
            ...
            Left_Key.SetButton(Input.GetKey((KeyCode)Left_Key.KeyCode));
            Right_Key.SetButton(Input.GetKey((KeyCode)Right_Key.KeyCode));
            for (int i = 0; i < MAX_PLAYERS; i++)
            {
            ...
            {
                Type t = AccessTools.TypeByName("IniFile.IniParser");
                if (t == null)
                {
                    DemulShooter_Plugin.MyLogger.LogError("Error");
                }
                else
                {
                    foreach (ConstructorInfo ci in AccessTools.TypeByName("IniFile.IniParser").GetConstructors())
                    {
                        ...
                        {
                            DemulShooter_Plugin.MyLogger.LogWarning(ci.Name + ", " + ci.GetParameters().Length);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < MAX_PLAYERS; i++)
            {
                OutputData.IsPlaying[i] = (byte)gamedata.m_playerdata[i].gamestate;
                OutputData.Life[i] = gamedata.m_playerdata[i].life;
                OutputData.Credits[i] = BufferCoin[i];
            }
