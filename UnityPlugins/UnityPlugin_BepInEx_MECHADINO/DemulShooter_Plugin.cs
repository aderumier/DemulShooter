...
namespace BepInEx_DemulShooter_Plugin
{
    ...
    {
        public const String pluginGuid = "argonlefou.demulshooter.mechdinosaur";
        public const String pluginName = "MechDinosaur_BepInEx_DemulShooter_Plugin";
        public const String pluginVersion = "17.0.0.0";
        public const String pluginConfigFile = "MechDinosaur_BepInEx_DemulShooter_Plugin.ini";
        ...
        private bool _EndListenerThread = false;
        ...
        //Storing requested Tickets to output, so that we can reply and ack the good number
        public static int[] RemainingTicketsOut = new int[MAX_PLAYERS];
        //Using static Buffer to fill when input is to be send to the game
        //Comparing with previous state in order to make the game decrypt buffer only onnew values
        //Otherrwise, game is decrypting the same buffer evry 10ms
        public static readonly int INPUT_BUFFER_LENGTH = 0x28;
        public static byte[] ReceivedDataBuffer = new byte[INPUT_BUFFER_LENGTH];
        public static byte[] ReceivedDataBuffer_Before = new byte[INPUT_BUFFER_LENGTH];
        ...
        public void Update()
        {
            //Scaling display if lower/bigger resolution
            //But need to filter both Width+Height because CanvasScaler can't scale differently X and Y and we would get weird result with 1920x1200 for example
            //When applying scale for 1200/1080 but also for 1920/1920 (= horizontal bug because of stretching)
            if (ForceResolution && ORIGINAL_HEIGHT != Screen.height && ORIGINAL_WIDTH != Screen.width)
                ScaleDisplay();
            ...
            if (Exit_Key.GetButtonDown())
                _EndListenerThread = true;

            //Getting some outputs
            for (int i = 0; i < Singleton<GameData>.Instance.GetTotalPlayer(); i++)
            {
                OutputData.IsPlaying[i] = Singleton<GameData>.Instance.GetPlayerLive(i) ? (byte)1 : (byte)0;
            }
        ...
        {
            float fScale = (float)Screen.height / ORIGINAL_HEIGHT;

            Canvas[] allCanvas = UnityEngine.Object.FindObjectsOfType<Canvas>();
            foreach (Canvas c in allCanvas)
            {
                foreach (UnityEngine.UI.CanvasScaler cs in c.GetComponents<UnityEngine.UI.CanvasScaler>())
                {
                    cs.scaleFactor = fScale;
                }
            }
        }
        ...
        private void TcpClientThreadLoop()
        {
            try
            {
                ...
                MyLogger.LogMessage(Instance.GetType().Name + "." + MethodBase.GetCurrentMethod().Name + "(): Terminating thread... ");
                Application.Quit();
            }
