...
namespace BepInEx_DemulShooter_Plugin
{
    ...
    {
        public const String pluginGuid = "argonlefou.demulshooter.dinoinvasion";
        public const String pluginName = "DinoInvasion_BepInEx_DemulShooter_Plugin";
        public const String pluginVersion = "17.0.0.0";
        public const String pluginConfigFile = "DinoInvasion_BepInEx_DemulShooter_Plugin.ini";
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < MAX_PLAYERS; i++)
            {
                ...
                try
                {
                    game_player Player = game_players.get_game_player(i);
                    if (Player != null)
                    {
                        OutputData.IsPlaying[i] = Player.my_is_user ? (byte)1 : (byte)0;
                        OutputData.Ammo[i] = Player.get_bullet_count();
                        OutputData.Credits[i] = (UInt16)game_system_control_insert_currency.my_coin_count_s[i];
                    }
                }
                catch (Exception) { }
                ; //Errors during intro screen
            }
